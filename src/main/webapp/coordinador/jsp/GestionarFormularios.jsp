...
<div class="sidebar">
    <div class="sidebar-content">
        ...
        <ul class="menu-links">
            ...
            <li><a href="GestionarFormulariosServlet"><i class="fa-solid fa-file-alt"></i> Gestionar Formularios</a></li> <%-- Puedes cambiar el texto si quieres --%>
...
<main class="contenedor-principal">
    <div class="main-content">
        ...
        <div class="encuestas-list">
            <%-- Cambiamos la iteración a "archivosCargados" --%>
            <c:choose>
                <c:when test="${not empty archivosCargados}">
                    <c:forEach var="archivo" items="${archivosCargados}">
                        <div class="encuesta-item">
                                <span class="encuesta-nombre">
                                    Archivo: ${archivo.nombreArchivoOriginal} <br>
                                    Ruta: ${archivo.rutaGuardado} <br>
                                    Cargado el: <fmt:formatDate value="${archivo.fechaCarga}" pattern="dd/MM/yyyy HH:mm"/> <br>
                                    Estado: ${archivo.estadoProcesamiento}
                                    <c:if test="${not empty archivo.mensajeProcesamiento}">
                                        <br> Mensaje: ${archivo.mensajeProcesamiento}
                                    </c:if>
                                    <c:if test="${archivo.idFormularioAsociado != null}">
                                        <br> Formulario Asociado ID: ${archivo.idFormularioAsociado}
                                    </c:if>
                                </span>
                                <%-- Los switches y botones de "Ingresar Respuesta" no tienen sentido directo para ArchivoCargado,
                                     a menos que cada archivo cargado represente un "formulario" que se puede activar/desactivar
                                     o que lleva a una "respuesta" (que sería la información detallada del archivo o su procesamiento).
                                     Si quieres mantener esa lógica, tendrías que definir qué significa "activo" o "ingresar respuesta"
                                     para un ArchivoCargado. Por ahora, los comento o quito para evitar confusión. --%>
                                <%--
                                <label class="switch">
                                    <input type="checkbox" <c:if test="${archivo.somePropertyForActive}">checked</c:if> disabled>
                                    <span class="slider"></span>
                                </label>
                                <button class="btn-ingresar" <c:if test="${!archivo.somePropertyForActive}">disabled</c:if>>Ver Detalles</button>
                                --%>
                        </div>
                    </c:forEach>
                </c:when>
